...
def download_and_resize_cover(url, output_dir=RESIZED_COVERS_DIR):
    ...
    try:
        ...
        output_path = output_dir / f"lock_screen_{filename}"
...
def main():
    ...
    covers_file = DATA_DIR / "covers.json"
    try:
        with open(covers_file, 'r') as f:
            covers = json.load(f)
    except FileNotFoundError:
        print(f"❌ {covers_file} not found. Please run scrape_thrasher_covers.py first.")
        return
